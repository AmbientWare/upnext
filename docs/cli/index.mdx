...
```bash
upnext server start [--host HOST] [--port PORT] [--backend BACKEND] [--database-url URL] [--redis-url URL] [--reload] [--verbose]
```

<Note>
Backend-first: persistence backend is selected by `--backend` or `UPNEXT_BACKEND`. Passing `--database-url` does not select `sqlite` or `postgres` by itself.
</Note>
...
```bash
# Start with Redis persistence backend (default)
UPNEXT_BACKEND=redis \
UPNEXT_REDIS_URL=redis://localhost:6379 \
upnext server start --port 8080

# Start with SQLite persistence backend via CLI flag
UPNEXT_DATABASE_URL=sqlite+aiosqlite:///upnext.db \
UPNEXT_REDIS_URL=redis://localhost:6379 \
upnext server start --backend sqlite

# Start with PostgreSQL persistence backend (run migrations first)
...
SQL migration commands powered by Alembic.
