...
Every UpNext runtime now operates inside a deployment namespace.

- The server uses `UPNEXT_DEFAULT_DEPLOYMENT_ID` and defaults to `local`.
- Worker and API processes use `UPNEXT_DEPLOYMENT_ID` and also default to `local`.

For hosted or multi-tenant runtimes, set both explicitly so Redis-backed runtime state stays isolated per deployment.
