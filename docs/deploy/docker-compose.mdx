...
<Steps>
  <Step title="Run database migrations (PostgreSQL backend only)">
    ```bash
  ...
  <Step title="Start the dashboard server">
    <Tabs>
      <Tab title="Redis backend (default)">
        ...
        UPNEXT_DEFAULT_DEPLOYMENT_ID=local \
...
<Note>
For self-hosted setups, `UPNEXT_API_KEY` is a single static bearer token. UpNext no longer creates local users or rotates keys through the dashboard.
</Note>
...
| `UPNEXT_DEFAULT_DEPLOYMENT_ID` | Deployment namespace served by the server (`local` by default) |
| `UPNEXT_DEPLOYMENT_ID` | Deployment namespace used by worker and API runtimes (`local` by default) |
