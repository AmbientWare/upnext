...
<Tabs>
  ...
  <Tab title="SDK + Redis + Server">
    ...
    | Authentication | Optional single-token gate for the dashboard and API |
    | Deployments | Deployment-scoped runtime state |

    <Note>
    Backend-first: set `UPNEXT_BACKEND` explicitly. The example below uses `sqlite`.
    For Redis vs PostgreSQL guidance, see [Persistence Backends](/concepts/backends).
    </Note>
    ...
    # Connect workers/APIs to the server
    UPNEXT_REDIS_URL=redis://localhost:6379 \
