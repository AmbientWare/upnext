...
UpNext provides automatic retries, timeout enforcement, and durable history for jobs that exhaust all attempts.
...
When a job exhausts all retry attempts, its final state is recorded as `failed` in job history. From the dashboard (or jobs API), you can inspect the failure details and trigger a manual retry of that same job ID.
