...
def _find_packaged_alembic_paths() -> tuple[Traversable, Traversable] | None:
    """Resolve migration assets packaged inside upnext-server."""
    try:
    ...
    try:
        ...
        error_panel(
            (
                ...
                "Install `upnext` (or `upnext-server`) and retry."
            ),
...
def _import_server_main() -> Path:
    ...
    try:
        ...
        if e.name in {"server", "server.main"}:
            error_panel(
                (
                    "Could not import `server.main:app`. Install `upnext` "
                    "(or `upnext-server`) and retry."
                ),
...
def _resolve_alembic_config() -> Iterator[AlembicConfig]:
    ...
    if packaged_paths is None:
        error_panel(
            (
                "Could not locate server migration files. Install `upnext` "
                "(or `upnext-server`). In a source checkout, run "
...
def _ensure_database_schema_ready(database_url: str | None, *, verbose: bool) -> None:
    ...
    try:
        missing_tables = asyncio.run(
            _get_missing_required_tables(effective_database_url)
        )
    except Exception as e:
...
def start(
    ...
    """
    Start the hosted UpNext server.
    """
    setup_logging(verbose=verbose)
...
def db_upgrade(
    ...
    """Run Alembic upgrade for the hosted UpNext server schema."""
