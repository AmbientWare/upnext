...
def _require_package_smoke_enabled() -> None:
    if os.getenv("UPNEXT_RUN_PACKAGE_SMOKE") != "1":
        pytest.skip(
            "Set UPNEXT_RUN_PACKAGE_SMOKE=1 to run package publish smoke tests."
        )
...
def _assert_all_modules_importable(python_bin: Path) -> None:
    _run(
        [
            str(python_bin),
            "-c",
            "import upnext,server,shared; print(upnext.__name__, server.__name__, shared.__name__)",
        ]
    )


def _assert_upnext_cli_entrypoint(venv_python: Path) -> None:
    upnext_bin = venv_python.parent / "upnext"
    assert upnext_bin.exists(), "upnext console script was not installed"
    _run([str(upnext_bin), "--help"])
...
def test_install_from_built_wheels_in_clean_venv(tmp_path: Path) -> None:
    ...
    _assert_upnext_cli_entrypoint(python_bin)
