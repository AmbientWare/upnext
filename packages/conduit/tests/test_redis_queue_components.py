...
from conduit.engine.queue.redis.fetcher import Fetcher
from conduit.engine.queue.redis.finisher import Finisher
from conduit.engine.queue.redis.queue import RedisQueue
from conduit.engine.queue.redis.sweeper import Sweeper
...
async def test_finisher_fallback_pipeline_flushes_and_acks(fake_redis) -> None:
    queue = RedisQueue(client=fake_redis, key_prefix="conduit-finisher-fallback")
    ...
    client = await queue._ensure_connected()  # noqa: SLF001
    pending = await client.xpending(queue._stream_key(job.function), queue._consumer_group)  # noqa: SLF001
...
async def test_sweeper_fallback_moves_due_job_to_stream(fake_redis) -> None:
    queue = RedisQueue(client=fake_redis, key_prefix="conduit-sweeper-fallback")
    queue._scripts_loaded = True  # noqa: SLF001
    queue._enqueue_sha = None  # noqa: SLF001
