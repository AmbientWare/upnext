...
async def test_metrics_always_recorded_and_event_sampling_respects_error_and_slow(
    ...
    minute_pattern = "conduit:api:orders:GET:/orders/{order_id}:m:*"
...
async def test_registry_refresh_only_writes_new_endpoint_without_periodic_refresh(
    ...
    registry_members = await fake_redis.smembers("conduit:api:registry")
    endpoints_members = await fake_redis.smembers("conduit:api:orders:endpoints")

    assert registry_members == {b"orders"}
    assert endpoints_members == {b"GET:/orders"}

    ttl = await fake_redis.ttl("conduit:api:orders:endpoints")
    assert ttl > 0
