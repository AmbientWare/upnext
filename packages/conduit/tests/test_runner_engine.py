...
import conduit.engine.runner as runner_module
...
async def test_run_services_ignores_duplicate_second_signal_within_debounce(
    ...
    def _unexpected_exit(code: int) -> None:
        raise AssertionError(f"os._exit({code}) should not be called for duplicate signal")
...
async def test_run_services_raises_when_service_crashes(monkeypatch) -> None:
    ...
    with pytest.raises(RuntimeError, match="Service 'api:api-crash' crashed; shutting down."):
...
@pytest.mark.asyncio
async def test_run_services_raises_when_service_exits_without_error(monkeypatch) -> None:
    loop_stub = _SignalLoopStub()
...
@pytest.mark.asyncio
async def test_run_services_cancels_pending_api_tasks_after_timeout(monkeypatch) -> None:
    loop_stub = _SignalLoopStub()
