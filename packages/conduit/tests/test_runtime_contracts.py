...
from conduit.engine import cron as cron_module
from conduit.engine.cron import calculate_next_cron_run, calculate_next_cron_timestamp
from conduit.engine.event_router import Event, get_event_router
...
def test_conduit_run_passes_components_and_timeouts(monkeypatch) -> None:
    ...
    monkeypatch.setattr(conduit, "run_services", fake_run_services)
    monkeypatch.setattr(conduit.asyncio, "run", run_coro)

    api = conduit.Api("api-test", redis_url=None)
    worker = conduit.Worker("worker-test", redis_url="redis://ignored")
    conduit.run(api, worker, worker_timeout=7.5, api_timeout=2.25)
...
def test_conduit_run_ignores_keyboard_interrupt(monkeypatch) -> None:
    ...
    monkeypatch.setattr(conduit, "run_services", fake_run_services)
    monkeypatch.setattr(conduit.asyncio, "run", raise_keyboard_interrupt)

    assert conduit.run() is None
...
def test_event_router_event_round_trip_and_singleton(monkeypatch) -> None:
    ...
    monkeypatch.setattr("conduit.engine.event_router._router", None)
    first = get_event_router()
    second = get_event_router()
    assert first is second


def test_cron_helpers_handle_naive_time_six_field_and_float_results(monkeypatch) -> None:
    ...
    monkeypatch.setattr(cron_module, "croniter", lambda *_args, **_kwargs: _FloatCroniter())
...
async def test_api_tracking_setup_and_cleanup(monkeypatch) -> None:
    ...
    monkeypatch.setattr("conduit.sdk.api.aioredis.from_url", lambda *_args, **_kwargs: fake_redis)
...
async def test_api_tracking_setup_handles_redis_connection_failure(monkeypatch) -> None:
    monkeypatch.setattr("conduit.sdk.api.aioredis.from_url", lambda *_args, **_kwargs: _BrokenRedis())
