...
@pytest.mark.asyncio
async def test_worker_writes_worker_and_function_definitions(fake_redis, monkeypatch) -> None:
    monkeypatch.setattr("conduit.sdk.worker.create_redis_client", lambda _url: fake_redis)
    monkeypatch.setattr("conduit.sdk.worker.JobProcessor", _NoopJobProcessor)
    ...
    try:
        ...
        function_def_raw = await fake_redis.get(f"{FUNCTION_KEY_PREFIX}:{handle.function_key}")
...
async def test_execute_helper_accepts_display_name_and_function_key(
    ...
    events = await fake_redis.xrange("conduit:status:events", count=20)
