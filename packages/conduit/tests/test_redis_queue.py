...
from conduit.engine.queue.base import DuplicateJobError
...
def queue(fake_redis):
    return RedisQueue(client=fake_redis, claim_timeout_ms=50, key_prefix="conduit-test")
...
@pytest.mark.asyncio
async def test_queue_lifecycle_enqueue_dequeue_finish_cleans_keys(queue: RedisQueue) -> None:
...
@pytest.mark.asyncio
async def test_retry_clears_stream_metadata_and_requeues_immediately(queue: RedisQueue) -> None:
...
async def test_xautoclaim_recovers_stale_message(fake_redis) -> None:
    q1 = RedisQueue(client=fake_redis, claim_timeout_ms=20, key_prefix="conduit-claim")
    q2 = RedisQueue(client=fake_redis, claim_timeout_ms=20, key_prefix="conduit-claim")
...
async def test_heartbeat_prevents_reclaim(fake_redis) -> None:
    q1 = RedisQueue(client=fake_redis, claim_timeout_ms=200, key_prefix="conduit-heartbeat")
    q2 = RedisQueue(client=fake_redis, claim_timeout_ms=200, key_prefix="conduit-heartbeat")
