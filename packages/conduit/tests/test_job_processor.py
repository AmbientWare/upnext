...
from conduit.engine.registry import Registry
...
class _StatusRecorder:
    ...
    async def record_job_log(self, job_id: str, level: str, message: str, **extra: Any) -> None:
...
async def test_context_commands_are_drained_to_backend() -> None:
    ...
    try:
        job = Job(function="task_key", function_name="task", kwargs={}, root_id="job-cmd")
        job.mark_started("worker")
...
async def test_terminal_failure_marks_failed_once() -> None:
    ...
    task_def = registry.register_task("task_key", task, retries=1, on_complete=on_complete)
    processor = JobProcessor(
        queue=cast(BaseQueue, queue), registry=registry, status_buffer=status
    )
...
async def test_execute_job_timeout_raises_with_job_id() -> None:
    ...
    try:
        job = Job(function="task_key", function_name="task", kwargs={}, root_id="job-timeout", timeout=0.01)
        job.mark_started("worker")
