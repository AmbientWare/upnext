"""Redis Streams queue implementation for Conduit.
...
class RedisQueue(BaseQueue):
    ...
    - Each function gets its own stream: conduit:fn:{function}:stream
    ...
    Key structure:
        conduit:fn:{function}:stream     - Stream for immediate jobs
    ...
    async def subscribe_job(self, job_id: str, timeout: float | None = None) -> str:
        ...
        try:
            ...
            await pubsub.unsubscribe(f"conduit:job:{job_id}")
            await pubsub.close()
