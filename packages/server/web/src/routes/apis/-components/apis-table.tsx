  ...
  return (
    <Table>
      ...
      <TableBody ref={bodyRef}>
        {apis.map((api) => {
          const goToApi = () => {
            navigate({ to: "/apis/$name", params: { name: api.name } });
          };
          ...
          return (
            ...
            >
              <TableCell className="py-2">
                <div className="flex flex-col gap-0.5 min-w-0">
                  <div className="inline-flex items-center gap-2 min-w-0">
                    ...
                    <span className="text-[11px] text-foreground truncate max-w-[190px] sm:max-w-none">
                      {api.name}
                    </span>
                  </div>
                  <span className="mono text-[10px] text-muted-foreground sm:hidden">
                    {api.endpoint_count} ep - {(api.instances ?? []).length} inst
                  </span>
                </div>
              </TableCell>
              ...
              <TableCell className="hidden sm:table-cell mono text-[11px] text-muted-foreground py-2">
                {api.endpoint_count}
              </TableCell>
