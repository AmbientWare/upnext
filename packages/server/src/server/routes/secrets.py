...
async def list_secrets(backend: BackendService = Depends(require_backend)):
    """List all secrets (key names only, no values)."""
    async with backend.session() as tx:
        secrets = await tx.secrets.list_secrets()
...
async def get_secret_by_name(
    ...
    async with backend.session() as tx:
        secret = await tx.secrets.get_secret_by_name(name)
        if secret is None:
...
async def get_secret(
    secret_id: str, backend: BackendService = Depends(require_backend)
):
    """Get a secret with decrypted values."""
    async with backend.session() as tx:
        secret = await tx.secrets.get_secret_by_id(secret_id)
        if secret is None:
...
async def create_secret(
    ...
    async with backend.session() as tx:
        try:
            secret = await tx.secrets.create_secret(body.name, body.data)
...
async def delete_secret(
    ...
    """Delete a secret."""
    async with backend.session() as tx:
        deleted = await tx.secrets.delete_secret(secret_id)
        if not deleted:
            raise HTTPException(status_code=404, detail="Secret not found")
